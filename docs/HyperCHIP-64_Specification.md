...
|FN3D|Sets the audio channel mask for the selected voice (N = 0 for No Audio Output, N = 1 for Channel 1 (Left), N = 2 for Channel 2 (Right); should the interpreter not be able to handle multi-channel audio, is free to treat it as a no-op)|HyperCHIP-64|N/A|
