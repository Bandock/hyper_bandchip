...
Hyper_BandCHIP::BandCHIP::XOCHIP_Machine::XOCHIP_Machine(unsigned int cycles_per_second, Renderer *DisplayRenderer) : CurrentResolutionMode(ResolutionMode::LoRes), DisplayRenderer(DisplayRenderer), delay_timer(0), sound_timer(0), PC(0), I(0), SP(0), draw_buffer(128 * 64), key_pressed(0), plane(0x01), rng_engine(system_clock::now().time_since_epoch().count()), rng_distrib(0, 255), cycle_accumulator(0.0), pause(true), operational(true), wait_for_key_release(false), error_state(MachineError::NoError)
{
...
Hyper_BandCHIP::BandCHIP::XOCHIP_Machine::~XOCHIP_Machine()
{
}

void Hyper_BandCHIP::BandCHIP::XOCHIP_Machine::ExecuteInstructions()
{
	...
	{
		SyncToCycle<XOCHIP_Machine, XOCHIP_Audio>(*this);
		uint8_t opcode = 0x0;
		...
		switch (opcode)
		{
			case 0x0:
			{
				switch (operand)
				{
					...
					case 0xEE:
					{
						...
						{
							PC -= 2;
							PauseProgram<XOCHIP_Machine, XOCHIP_Audio>(*this, true);
			...
			case 0x8:
			{
				...
				switch (z)
				{
					...
					case 0x6:
					{
						uint8_t &shift = (behavior_data.SuperCHIP_Shift ? V[x] : V[y]);
						uint8_t tmp = (shift & 0x01);
						V[x] = (shift >> 1);
						V[0xF] = tmp;
						break;
					}
			...
			case 0xD:
			{
				...
				if (!height)
				{
					if (CurrentResolutionMode == ResolutionMode::HiRes || (CurrentResolutionMode == ResolutionMode::LoRes && behavior_data.Octo_LoResSprite))
					{
						width = 16;
					}
					height = 16;
			...
			case 0xF:
			{
				...
				switch (opcode2)
				{
					...
					case 0x02:
					{
						audio_system.CopyToAudioBuffer(memory.data(), I);
						break;
					}
					case 0x07:
					{
					...
					case 0x18:
					{
						SetSoundTimer<XOCHIP_Machine, XOCHIP_Audio>(*this, V[x]);
						break;
					}
					case 0x1E:
					{
					...
					{
						uint8_t pitch = V[x];
						audio_system.SetPlaybackRate(pitch);
						break;
					}
					case 0x55:
					{
						...
						if (!behavior_data.SuperCHIP_LoadStore)
						{
							I += x + 1;
						}
						break;
